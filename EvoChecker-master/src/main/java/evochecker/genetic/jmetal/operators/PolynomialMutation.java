...
public class PolynomialMutation extends Mutation {
   ...
   */
  private static final List VALID_TYPES = Arrays.asList(RealSolutionType.class, ArrayRealSolutionType.class, ArrayRealIntSolutionType.class) ;
...
//		System.out.println("Type is = " + (solution.getType().getClass().equals(ArrayRealIntSolutionType.class)));
		if(solution.getType().getClass().equals(ArrayRealIntSolutionType.class)) {
			XInt intPart = new XInt(solution) ;
			...
			for (int var=0; var < intPart.getNumberOfDecisionVariables(); var++) {
				if (PseudoRandom.randDouble() <= probability)
				{
					y      = intPart.getValue(var);
					yl     = intPart.getLowerBound(var);                
					yu     = intPart.getUpperBound(var);
					delta1 = (y-yl)/(yu-yl);
					delta2 = (yu-y)/(yu-yl);
					rnd = PseudoRandom.randDouble();
					mut_pow = 1.0/(eta_m_+1.0);
					...
					if (y>yu)
						y = yu;
					intPart.setValue(var, (int) Math.round(y));                           
				}
			} // for
